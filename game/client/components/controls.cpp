...
int CControls::SnapInput(int *pData)
{
	...
	else
	{
		...
		if(g_Config.m_ClPlusTripleDummyFly)
		{
			// Get pointer to dummy input
			CNetObj_PlayerInput *pDummyInput = &m_pClient->m_DummyInput;

			// Copy basic movement and state data
			pDummyInput->m_Hook = m_aInputData[g_Config.m_ClDummy].m_Hook;
			pDummyInput->m_PlayerFlags = m_aInputData[g_Config.m_ClDummy].m_PlayerFlags;
			pDummyInput->m_TargetX = m_aInputData[g_Config.m_ClDummy].m_TargetX;
			pDummyInput->m_TargetY = m_aInputData[g_Config.m_ClDummy].m_TargetY;
			...
			// Copy input data to non-dummy slot
			m_aInputData[!g_Config.m_ClDummy] = *pDummyInput;
		}

		if(g_Config.m_ClPlusStabilizer)
			m_pClient->m_PlusFeatures.Stabilizer();

		if(g_Config.m_ClPlusSpinbot)
			m_pClient->m_PlusFeatures.Spinbot();

		if(g_Config.m_ClPlusRecording)
		{
			if(!g_Config.m_ClPlusWasRecording)
			{
				// Clear all recording vectors
				m_pClient->m_PlusFeatures.records.recordsActions.clear();
				m_pClient->m_PlusFeatures.records.recordsMouse.clear();
				m_pClient->m_PlusFeatures.records.recordsPositions.clear();
				m_pClient->m_PlusFeatures.records.recordsDummySwitch.clear();
				m_pClient->m_PlusFeatures.records.recordsKill.clear();
				g_Config.m_ClPlusWasRecording = 1;
			}
			m_pClient->m_PlusFeatures.Record();
		}
		else if(g_Config.m_ClPlusPlaying)
		{
			if(!m_pClient->m_PlusFeatures.records.recordsActions.empty() && 
			   !m_pClient->m_PlusFeatures.records.recordsMouse.empty())
			{
				m_pClient->m_PlusFeatures.Play();
				if(m_pClient->m_PlusFeatures.records.recordsActions.empty() || 
				   m_pClient->m_PlusFeatures.records.recordsMouse.empty())
				{
		...
		if(g_Config.m_ClPlusPet)
			m_pClient->m_PlusFeatures.PetStateUpdate();

		if(g_Config.m_ClPlusAutoWallshot)
			m_pClient->m_PlusFeatures.AutoWallshot();

		if(g_Config.m_ClPlusAutoAim)
			m_pClient->m_PlusFeatures.AutoAim();

		if(g_Config.m_ClPlusHammerCopy)
			m_pClient->m_PlusFeatures.HammerCopy();

		if(g_Config.m_ClPlusFlyBot)
		{
			m_pClient->m_PlusFeatures.FlyBot();
		}
		else if(m_pClient->m_Controls.m_FlyInfo.Posed)
		{
			m_pClient->m_Controls.m_FlyInfo.Posed = 0;
		}

		if(m_pClient->m_PlusFeatures.NeedReset)
		{
			m_aInputData[g_Config.m_ClDummy].m_Jump = 0;
			m_aInputData[g_Config.m_ClDummy].m_Direction = 0;
			m_pClient->m_PlusFeatures.NeedReset = 0;
		}

		if(g_Config.m_ClPlusAntiFallingFreeze)
			m_pClient->m_PlusFeatures.AntiFallingFreeze();

		if(g_Config.m_ClPlusAntiGoingFreeze)
		{
			m_pClient->m_PlusFeatures.AntiGoingFreeze(g_Config.m_ClPlusAntiGoingFreezeType);
		}

		if(g_Config.m_ClMoonWalk)
		{
			if(m_aInputDirectionRight[g_Config.m_ClDummy])
			{
				if(m_LastSlowMove <= 0 || 
				m_LastSlowMove + Client()->GameTickSpeed() / g_Config.m_ClMoonWalkSpeed <= Client()->GameTick(g_Config.m_ClDummy))
				{
					m_aInputData[g_Config.m_ClDummy].m_Direction = 1;
					m_LastSlowMove = Client()->GameTick(g_Config.m_ClDummy);
				}
				else
					m_aInputData[g_Config.m_ClDummy].m_Direction = 0;
			}
			else if(m_aInputDirectionLeft[g_Config.m_ClDummy])
			{
				if(m_LastSlowMove <= 0 || 
				m_LastSlowMove + Client()->GameTickSpeed() / g_Config.m_ClMoonWalkSpeed <= Client()->GameTick(g_Config.m_ClDummy))
				{
